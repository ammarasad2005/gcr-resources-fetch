...
function isConvertibleToPdf(file) {
  ...
  if (file.driveFileId) {
    ...
    return convertibleExts.includes(ext);
  }
  return false;
}
...
async function convertAndFetchPdfBlob(file) {
  ...
  if (['ppt', 'pptx'].includes(ext)) targetMimeType = 'application/vnd.google-apps.presentation';
  if (['xls', 'xlsx', 'csv'].includes(ext)) targetMimeType = 'application/vnd.google-apps.spreadsheet';

  // 1. Copy to convert
  const copyRes = await apiFetchJSON(
    `https://www.googleapis.com/drive/v3/files/${encodeURIComponent(file.driveFileId)}/copy?supportsAllDrives=true`,
    'POST',
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (chrome.runtime.lastError || !response || !response.success) {
          ...
          alert(`PDF Conversion API Error: ${err}\nURL: ${url}`);
          resolve(null);
        } else {

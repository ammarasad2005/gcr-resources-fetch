...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  links.forEach((link, idx) => {
    ...
    relsEntries += `<Relationship Id="rId_link${idx}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="${escapedUrl}" TargetMode="External"/>\n`;
  });
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  links.forEach((link, idx) => {
    ...
    if (groups[t]) {
      groups[t].items.push({ ...link, relId: `rId_link${idx}` });
    } else {
      groups['link'].items.push({ ...link, relId: `rId_link${idx}` });
    }
  });

...
async function convertAndFetchPdfBlob(file) {
  ...
  let blob = null;
  try {
    blob = await fetchFileBlob(pdfUrl);
  } catch (err) {
    if (window.showToast) window.showToast(`PDF Export failed: ${err}`, 'error');
  }

  if (!blob && window.showToast) {
    window.showToast(`PDF Export failed to fetch blob for converted file`, 'error');
  }
